// components/CompletedQuests.tsx
"use client";
...
export default function CompletedQuests() {
    const [completedQuests, setCompletedQuests] = useState<Quest[] | null>(null);
    ...
    useEffect(() => {
        async function loadCompleted() {
            try {
                const res = await fetch("/api/quests/completed-quests", {
                    cache: "no-store",
                });
                if (!res.ok) throw new Error("Failed to fetch completed quests");

                // API returns array of objects that include a nested `quest` field.
                const data = await res.json();
                const quests: Quest[] = data.map((item: any) => ({
                    id: item.quest.id,
                    title: item.quest.title,
                    imgUrl: item.quest.imgUrl,
                }));
    ...
    if (isError) {
        return (
            <div className="mt-6 p-4 text-red-500">
                Failed to retrieve Completed Quests
            </div>
        );
    }
    ...
    if (!completedQuests || completedQuests.length === 0) {
        return (
            <div className="mt-6 p-4">
                It is time to start your journey to explore Tokyo with us!!
            </div>
        );
    }
